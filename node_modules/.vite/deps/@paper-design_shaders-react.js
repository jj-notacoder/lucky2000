...
var ShaderMount = class {
  constructor(parentElement, fragmentShader, uniforms, webGlContextAttributes, speed = 0, frame = 0, minPixelRatio = 2, maxPixelCount = DEFAULT_MAX_PIXEL_COUNT, mipmaps = []) {
    ...
    if ((parentElement == null ? void 0 : parentElement.nodeType) === 1) {
...
function toProcessedGemSmoke(file) {
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = async () => {
      ...
      const minDimension = Math.min(originalWidth, originalHeight);
      const targetSize = POISSON_CONFIG_OPTIMIZED2.workingSize;
      const scaleFactor = targetSize / minDimension;
      const width = Math.round(originalWidth * scaleFactor);
      const height = Math.round(originalHeight * scaleFactor);
      if (POISSON_CONFIG_OPTIMIZED2.measurePerformance) {
        console.log(`[Processing Mode]`);
        console.log(`  Original: ${originalWidth}×${originalHeight}`);
        console.log(`  Working: ${width}×${height} (${(scaleFactor * 100).toFixed(1)}% scale)`);
        if (scaleFactor < 1) {
      ...
      const sparseData = buildSparseData2(
        shapeMask,
        boundaryMask,
        new Uint32Array(interiorIndices),
        new Uint32Array(boundaryIndices),
      ...
      const startSolve = performance.now();
      const u = solvePoissonSparse2(sparseData, shapeMask, boundaryMask, width, height);
      if (POISSON_CONFIG_OPTIMIZED2.measurePerformance) {
        console.log(`[Poisson Solve] Time: ${(performance.now() - startSolve).toFixed(2)}ms`);
      }
      let maxVal = 0;
...
function buildSparseData2(shapeMask, boundaryMask, interiorPixels, boundaryPixels, width, height) {
  const pixelCount = interiorPixels.length;
  const neighborIndices = new Int32Array(pixelCount * 4);
  for (let i = 0; i < pixelCount; i++) {
    const idx = interiorPixels[i];
...
function solvePoissonSparse2(sparseData, shapeMask, boundaryMask, width, height) {
  const ITERATIONS = POISSON_CONFIG_OPTIMIZED2.iterations;
  const C = 0.01;
  const u = new Float32Array(width * height);
  const { interiorPixels, neighborIndices, pixelCount } = sparseData;
  const startTime = performance.now();
...
// node_modules/@paper-design/shaders/dist/empty-pixel.js
...
// node_modules/@paper-design/shaders-react/dist/shaders/gem-smoke.js
...
var gemSmokePresets = [defaultPreset29, firePreset, fluorescentPreset, infraredPreset];
var GemSmoke = (0, import_react30.memo)(function GemSmokeImpl({
  // Own props
  colorBack = defaultPreset29.params.colorBack,
  colors = defaultPreset29.params.colors,
  speed = defaultPreset29.params.speed,
  frame = defaultPreset29.params.frame,
  image = "",
  innerDistortion = defaultPreset29.params.innerDistortion,
  outerDistortion = defaultPreset29.params.outerDistortion,
  outerGlow = defaultPreset29.params.outerGlow,
  innerGlow = defaultPreset29.params.innerGlow,
  colorInner = defaultPreset29.params.colorInner,
  offset = defaultPreset29.params.offset,
  angle = defaultPreset29.params.angle,
  size = defaultPreset29.params.size,
  shape = defaultPreset29.params.shape,
  suspendWhenProcessingImage = false,
  // Sizing props
  fit = defaultPreset29.params.fit,
  scale = defaultPreset29.params.scale,
  rotation = defaultPreset29.params.rotation,
  originX = defaultPreset29.params.originX,
  originY = defaultPreset29.params.originY,
  offsetX = defaultPreset29.params.offsetX,
  offsetY = defaultPreset29.params.offsetY,
  worldWidth = defaultPreset29.params.worldWidth,
  worldHeight = defaultPreset29.params.worldHeight,
  ...props
}) {
  const imageUrl = typeof image === "string" ? image : image.src;
  const [processedStateImage, setProcessedStateImage] = (0, import_react30.useState)(transparentPixel);
  let processedImage;
  if (suspendWhenProcessingImage && typeof window !== "undefined" && imageUrl) {
    processedImage = suspend(
